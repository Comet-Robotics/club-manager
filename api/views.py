...
class CombatTeamViewSet(viewsets.ModelViewSet):
    ...
    @extend_schema(
        responses={
            ...
            400: {
                "type": "object",
                "properties": {"detail": {"type": "string", "enum": ["You are already a manager for this team."]}},
                "required": ["detail"],
            },
        },
...
class CombatEventViewSet(viewsets.ModelViewSet):
    ...
    def _upsert_teams_with_rce_data(self, rce_teams: list[RCETeam]) -> dict[str, CombatTeam]:
        ...
        CombatTeam.objects.bulk_create(
            combat_teams, update_conflicts=True, unique_fields=["robot_combat_events_team_id"], update_fields=["name"]
        )
    ...
    def _associate_robots_with_event(
        self, combat_robots: list[CombatRobot], combat_event: CombatEvent, rce_robots: list[RCERobot]
    ):
        ...
        CombatEventRegistration.objects.bulk_create(
            reg_objs, update_conflicts=True, unique_fields=["combat_event", "combat_robot"], update_fields=["status"]
        )
    ...
    @extend_schema(
        responses={
            ...
            404: {
                "type": "object",
                "properties": {"detail": {"type": "string", "enum": ["CombatEvent not found."]}},
                "required": ["detail"],
            },
        },
...
class RobotsInEventView(APIView):
    @extend_schema(
        responses={200: CombatRobotSerializer(many=True), 403: CombatRobotSerializer(many=True)},
        description="Get teams in a combat event",
    )

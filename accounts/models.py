...
def validate_after_registration_redirect_destination(value: str | None) -> None:
  ...
  if not (
    ...
    raise ValidationError(
      "After-registration redirect destinations must be site-relative paths."
    )
  ...
  def purge_deletion_candidates():
    with transaction.atomic():
      return User.objects.filter(
        pk__in=UserStub._get_deletion_candidates().values("user_id")
      ).delete()
  ...
  def create(
    ...
    with transaction.atomic():
      ...
      if discord_user_id is not None:
        ...
        profile.save()
      user_stub = UserStub.objects.create(user=user, after_registration_redirect_destination=after_registration_redirect_destination)
      return user_stub

...
async def get_user_attendances(user_profile: UserProfile):
    def get_attendances():
        return sorted(
            Attendance.objects.filter(user=user_profile.user),
            key=lambda a: a.event.event_date,
            reverse=True,
        )
...
async def link(ctx: discord.ApplicationContext, net_id: str):
    ...
    if profile.discord_id:
        if profile.discord_id == str(author_id):
            await ctx.respond(
                "That Net ID is already linked to your account!",
                ephemeral=True,
            )
        else:
            await ctx.respond(
...
"""
    ...
    await ctx.respond(
        embed=embed,
        ephemeral=True,
        view=ProfileActionsView(term_name, due_paying_url, user_profile),
    )
...
async def version(ctx: discord.ApplicationContext):
    ...
    try:
        ...
        await ctx.respond(
            f"An error occurred while fetching version information: {str(e)}",
            ephemeral=True,
        )
